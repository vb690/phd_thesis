...
We will report here frequent notation used through all the thesis. Each notation is referenced when it appears in the manuscript main body for the first time.
...
\end{gather}
with $\sigma$  being the sigmoid function, $\tanh$ the hyperbolic function, $\theta_{xf}$, $\theta_{xi}$, $\theta_{xo}$, $\theta_{xc}$ $\in \mathbb{R}^{B \times h}$ and $\theta_{hf}$,  $\theta_{hi}$,  $\theta_{ho}$, $\theta_{hc}$, $\theta_{hf}$ $\in \mathbb{R}^{h \times h}$ a set of learnable weights matrices, $\beta_f$, $\beta_i$, $\beta_o$, $\beta_c$ $\in \mathbb{R}^{h}$ , a set of learnable biases, $c_t$ the value at time $t$ of the conveyor belt matrix and $h_t$ the value at time $t$ of the hidden state matrix.
